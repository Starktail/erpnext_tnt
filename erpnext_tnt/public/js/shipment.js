frappe.ui.form.on("Shipment", {
  ...
  fetch_tnt_shipping_rates: function (frm) {
    if (!frm.doc.shipment_id) {
      frappe.call({
        method:
          "erpnext_tnt.tnt.doctype.tnt_shipment.tnt_shipment.create_tnt_shipment_doc_and_request_rates",
        freeze: true,
        args: {
...
function select_from_available_tnt_services(frm, tnt_data) {
  ...
  if (default_service) {
    rates.sort((a, b) => {
      if (
        a.product.id === default_service &&
        b.product.id !== default_service
      ) {
        return -1;
      }
      if (
    ...
    fields: [
      {
        ...
        label: "",
        options: tnt_data.is_hazardous
          ? __(
              "<span class='text-danger'>Hazardous shipments are not supported for rate calculation</span>",
            )
          : "",
      },
      {
  ...
  frm.select_row = function (service_data) {
    frappe.call({
      method:
        "erpnext_tnt.tnt.doctype.tnt_shipment.tnt_shipment.book_tnt_shipment",
      freeze: true,
